    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            if s == 0:
                ...
                frames=np_img
                frames=frames.reshape(palette_height.value, palette_width.value, 3)
                #imgs = cv2.cvtColor(imgs, cv2.COLOR_BGR2RGB)
                #cv2.imshow('image',immgs)
                #cv2.waitKey(5000)
                #print(immgs.shape)
                
                #cap=libir.evo_irimager_get_thermal_palette_image_metadata(thermal_width, thermal_height, npThermalPointer, palette_width, palette_height, npImagePointer, ct.byref(metadata))
                ...
                self.imgs[i] = frames # guarantee first frame
